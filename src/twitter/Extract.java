...
public class Extract {
	...
	public static Timespan getTimespan(List<Tweet> tweets) {
	 ...
	 // Initialize the start and end times based on the first tweet in the list
	    Instant start = tweets.get(0).getTimestamp();
	    Instant end = tweets.get(0).getTimestamp();
	    
	    // Loop through the list of tweets to find the minimum and maximum timestamps
	    ...
	    // Return the timespan without modifying the input list	    
	...
	public static Set<String> getMentionedUsers(List<Tweet> tweets) {
	    ...
	    for (Tweet tweet : tweets) {
	        ...
	        while (matcher.find()) {
	            String username = matcher.group().substring(1); // remove '@'
